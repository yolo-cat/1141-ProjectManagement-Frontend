<template>
  <div class="container nb-brutal">
    ...
    <form v-if="form" @submit.prevent="submit" class="nb-brutal">
      <label>姓氏</label>
      <input v-model.trim="form.lastName" required maxlength="100" class="input nb-brutal" />

      <label>名字</label>
      <input v-model.trim="form.firstName" required maxlength="100" class="input nb-brutal" />

      <label>電子郵件</label>
      <input v-model.trim="form.email" type="email" required maxlength="150" class="input nb-brutal" />

      <label>出生日期</label>
      <input v-model.trim="form.dateOfBirth" type="date" required class="input nb-brutal" />

      <div class="actions">
        <button class="btn nb-brutal" type="submit">更新資料</button>
        <router-link class="btn nb-brutal" to="/student/dashboard">返回儀表板</router-link>
      </div>
